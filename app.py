...
def tau_c(pt, fck):
    """Approximate tau_c (MPa). Good enough for project demonstration."""
    pt = max(pt, 0.1)
    return 0.62 * math.sqrt(fck) / (1 + 1.5 / pt)

def calculate_capacity(fck, fy, b, D, L, load_type,
                       main_dia, main_count, stirrup_dia, spacing, cover):
    ...
    Asv = (math.pi / 4) * (stirrup_dia ** 2) * 2   # two-leg stirrup
    ...
    Mu = 0.36 * fck * b * xu * (d - 0.42 * xu)      # N·mm
    Mu_lim = 0.138 * fck * b * d ** 2
    Mu = min(Mu, Mu_lim)

    # Convert moment to load
...
L = st.number_input("Clear Span L (mm)", value=4000)

load_type = st.selectbox("Loading Type", ["Point Load", "Two Point Load"])
...
cover = st.number_input("Clear Cover (mm)", value=25)
